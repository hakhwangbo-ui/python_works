...
try:
    ...
    sample = [
        ['2026-04-01', '식비',120000,'점심'],
        ['2026-04-01', '교통',2500,'지하철'],
        ['2026-04-02', '카페',15500,'커피'],
        ['2026-04-03', '식비',180000,'저녁'],
        ['2026-04-04', '쇼핑',430000,'생활용품'],
    ]
    df = pd.DataFrame(sample, columns=['date','category','amount','memo'])
print("\n[지출내역]")
print(df)
...
print(f"총 지출: {monthly_total:,}")

# 간단한 AI 스타일 분석 규칙
advice = [] # 빈 리스트 생헝
category_sum = df.groupby('category')['amount'].sum().to_dict() # 딕셔너리로 변환
print(category_sum) #{'교통': 2500, '쇼핑': 430000, '식비': 300000, '카페': 5500}

print(category_sum.get('카페', 0)) #5500
...
if category_sum.get("쇼핌", 0) > 3000:
    advice.append("쇼핑 지출이 큽니다. 필요/충동 구매를 구분해 보세요") 
if not advice:
    advice.append("지출 패턴이 안정적입니다. 현재 소비 습관을 유지하세요.")       

#print(advice)

# 1부터 시작하는 번호와 함계 조인 출력
